...
def tinytag_convert(tinytag_raw_df, sensor_name):
    ...
    try:
        ...
        tinytag_df = tinytag_raw_df[
            [CONST_TINYTAG_COL_SENSOR_NAME, CONST_TINYTAG_COL_TIMESTAMP]
        ].copy()
        ...
        tinytag_df.rename(
            columns={
                CONST_TINYTAG_COL_SENSOR_NAME: "temperature",
    ...
    try:
        tinytag_df[CONST_TINYTAG_COL_TIMESTAMP] = pd.to_datetime(
            tinytag_df[CONST_TINYTAG_COL_TIMESTAMP], format="%Y-%m-%dT%H:%M:%S.%f",
        )
        tinytag_df[CONST_TINYTAG_COL_TEMPERATURE] = tinytag_df[
            CONST_TINYTAG_COL_TEMPERATURE
        ].astype("float64")
